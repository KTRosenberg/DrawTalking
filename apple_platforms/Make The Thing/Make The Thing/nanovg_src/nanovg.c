...
typedef struct NVG_Transform {
    float matrix[16];
} NVG_Transform;

#define NVG_TRANSFORM_STACK_MAX_COUNT (127)
typedef struct NVG_Transform_Stack {
    NVG_Transform stack[NVG_TRANSFORM_STACK_MAX_COUNT + 1];
    unsigned int cursor;
} NVG_Transform_Stack;


inline static void NVG_Transform_Stack_reset(NVG_Transform_Stack* stack)
{
    stack->cursor = 1;
}

inline static void set_identity(float transform[16])
{
...
void NVG_Transform_Stack_init(NVG_Transform_Stack* stack)
{
    memset(stack, 0, sizeof(NVG_Transform_Stack));
    set_identity(stack->stack[0].matrix);
    NVG_Transform_Stack_reset(stack);
}
...
struct NVGcontext {
    ...
    NVG_Transform_Stack model_transform_stack;
};

static NVGcontext* nvg__global_context;
...
NVGcontext* nvgCreateInternal(NVGparams* params)
{
    ...
    params->renderSetHWScissor = NULL;
    
    NVG_Transform_Stack_init(&ctx->model_transform_stack);
...
void nvgSetupDraws(NVGcontext* ctx)
{
    ctx->params.renderSetupDraws(ctx->params.userPtr);
}

void nvgDrawRange(NVGcontext* ctx, unsigned long long first_index, unsigned long long count)
{
    ctx->params.renderDrawRange(ctx->params.userPtr, first_index, count);
}

void nvgDraw(NVGcontext* ctx)
{
    ctx->params.renderDraw(ctx->params.userPtr);
}
...
void nvgFlushOnly(NVGcontext* ctx)
{
    ctx->params.renderFlushOnly(ctx->params.userPtr);
}

void nvgEndFrameOnly(NVGcontext* ctx)
{
    ctx->params.renderEndFrameOnly(ctx->params.userPtr);
...
static NVGpoint* nvg__addPoint(NVGcontext* ctx, float x, float y, int flags)
{
...
static int nvg__curveDivsEX(float64 r, float64 arc, float64 tol)
{
    float64 da = acos(r / (r + tol)) * 2.0f;
    return nvg__maxi(4, (int)ceil(arc / da));
}
...
static NVGvertex* nvg__roundCapStartEX(NVGvertex* dst, NVGpoint* p,
                                     float dx, float dy, float w, int ncap,
                                     float aa, float u0, float u1)
{
    int i;
    ...
    NVG_NOTUSED(aa);
    for (i = 0; i < ncap; i++) {
        float64 a = (float64)i/(float64)(ncap-1)*NVG_PI;
        float64 ax = cos(a) * w, ay = sin(a) * w;
        nvg__vset(dst, px - dlx*ax - dx*ay, py - dly*ax - dy*ay, u0,1); dst++;
        nvg__vset(dst, px, py, 0.5f,1); dst++;
    }
    nvg__vset(dst, px + dlx*w, py + dly*w, u0,1); dst++;
...
static NVGvertex* nvg__roundCapEndEX(NVGvertex* dst, NVGpoint* p,
                                   float dx, float dy, float w, int ncap,
                                   float aa, float u0, float u1)
{
    int i;
...
static void nvg__calculateJoinsEX(NVGcontext* ctx, float w, int lineJoin, float miterLimit)
{
    NVGpathCache* cache = ctx->cache;
    int i, j;
    float iw = 1.0f;
...
static int nvg__expandStrokeEX(NVGcontext* ctx, float w, float fringe, int lineCap, int lineJoin, float miterLimit)
{
    NVGpathCache* cache = ctx->cache;
    NVGvertex* verts;
    NVGvertex* dst;
    ...
    nvg__calculateJoinsEX(ctx, w, lineJoin, miterLimit);
    ...
    for (i = 0; i < cache->npaths; i++) {
        ...
        NVGpoint* pts = &cache->points[path->first];
        int max_ncap = 0;
        for (int j = 0; j < path->count; j += 1) {
            int ncap = nvg__curveDivsEX(pts[j].width, NVG_PI, ctx->tessTol);
            max_ncap = (ncap > max_ncap) ? ncap : max_ncap;
            pts[j].ncap = ncap;
        }
        int ncap = max_ncap;
    ...
    for (i = 0; i < cache->npaths; i++) {
        ...
        if (loop == 0) {
            ...
            if (lineCap == NVG_BUTT)
                ...
                w = p0->width;
                dst = nvg__roundCapStartEX(dst, p0, dx, dy, w, p0->ncap, aa, u0, u1);
            }
        }
        ...
        for (j = s; j < e; ++j) {
            if ((p1->flags & (NVG_PT_BEVEL | NVG_PR_INNERBEVEL)) != 0) {
                if (lineJoin == NVG_ROUND) {
                    w = p0->width;
                    dst = nvg__roundJoin(dst, p0, p1, w, w, u0, u1, p0->ncap, aa);
                } else {
                    w = p0->width;
                    dst = nvg__bevelJoin(dst, p0, p1, w, w, u0, u1, aa);
                }
        ...
        if (loop) {
            ...
            if (lineCap == NVG_BUTT)
                ...
                w = p0->width;
                dst = nvg__roundCapEndEX(dst, p1, dx, dy, w, p0->ncap, aa, u0, u1);
            }
        }
...
void nvgLineToEX(NVGcontext* ctx, float x, float y, float z, float width, NVGcolor color)
{
    float vals[] = { NVG_LINETO_EX, x, y, z, width, color.r, color.g, color.b, color.a};
    nvg__appendCommands(ctx, vals, NVG_COUNTOF(vals));
}
...
void nvgStrokeEX(NVGcontext* ctx)
{
    NVGstate* state = nvg__getState(ctx);
    float scale = nvg__getAverageScale(state->xform);
    float strokeWidth = nvg__clampf(state->strokeWidth * scale, 0.0f, 200.0f);
...
//        nvg__expandStroke(ctx, strokeWidth*0.5f, ctx->fringeWidth, state->lineCap, state->lineJoin, state->miterLimit);
//    else
        nvg__expandStrokeEX(ctx, strokeWidth*0.5f, 0.0f, state->lineCap, state->lineJoin, state->miterLimit);
...
void nvgTextScale(NVGcontext* ctx, float scale) {
    ctx->textScale = nvg__clampf(scale, 1.0, 8.0);
}
...
float nvgText(NVGcontext* ctx, float x, float y, const char* string, const char* end)
{
	...
	FONSquad q;
	NVGvertex* verts;
	float scale = nvg__getFontScale(state) * ctx->devicePxRatio * ctx->textScale;
...
void nvgSetViewTransform(NVGcontext* ctx, float transform[16])
{
    ctx->params.renderSetViewTransform(ctx->params.userPtr, transform);
}

void nvgSetModelTransform(NVGcontext* ctx, float transform[16])
{
    ctx->params.renderSetModelTransform(ctx->params.userPtr, transform);
}

void nvgSetHWScissor(NVGcontext* ctx, uint64 x, uint64 y, uint64 width, uint64 height)
{
    float scale = ctx->devicePxRatio;
    ctx->params.renderSetHWScissor(ctx->params.userPtr, x * scale, y * scale, width * scale, height * scale);
}


void nvgSetGlobalContext(NVGcontext* ctx)
{
    nvg__global_context = ctx;
}
NVGcontext* nvgGetGlobalContext(void)
{
    return nvg__global_context;
}

void NVG_save_model_transform(NVGcontext* ctx, float transform[16])
{
    NVG_Transform_Stack* stack = &ctx->model_transform_stack;
    if (stack->cursor > NVG_TRANSFORM_STACK_MAX_COUNT) {
        return;
    }
    
    memcpy((ctx->model_transform_stack.stack[ctx->model_transform_stack.cursor].matrix), transform, sizeof(float) * 16);
    
    stack->cursor += 1;
}
void NVG_restore_model_transform(NVGcontext* ctx, float transform[16])
{
    NVG_Transform_Stack* stack = &ctx->model_transform_stack;
    if (stack->cursor == 1) {
        return;
    ...
    if (transform != NULL) {
        NVG_model_transform_top(ctx, transform);
    }
}

void NVG_model_transform_top(NVGcontext* ctx, float transform_out[16])
{
    memcpy(transform_out, (ctx->model_transform_stack.stack[ctx->model_transform_stack.cursor - 1].matrix), sizeof(float) * 16);
}

void NVG_foreach_point(NVGcontext* ctx, int idx, void (*cb)(void* state, NVGvertex* vertex), void* state)
{
    if (idx >= ctx->cache->npaths) {
        return;
    }
    
    NVGpath* path = &ctx->cache->paths[idx];
    for (int i = 0; i < path->nstroke; i += 1) {
        cb(state, &path->stroke[i]);
    }
}


NVGpoint* nvg__addPoint_TEST(NVGcontext* ctx, NVG_PointListElement* el, int flags)
{
    NVGpath* path = nvg__lastPath(ctx);
    NVGpoint* pt;
    if (path == NULL) return NULL;
    ...
    if (path->count > 0 && ctx->cache->npoints > 0) {
        pt = nvg__lastPoint(ctx);
        if (nvg__ptEquals(pt->x,pt->y, el->x,el->y, ctx->distTol)) {
            pt->flags |= flags;
            return NULL;
        }
    }
...
static void nvg__flattenPathsWithPointList(NVGcontext* ctx, NVG_PointListElement* list, size_t count)
{
    NVGpathCache* cache = ctx->cache;
//    NVGstate* state = nvg__getState(ctx);
    NVGpoint* last;
...
//    nvg__addPath(ctx);
    ...
    for (i = 0; i < count; i += 1) {
        nvg__addPoint_TEST(ctx, &list[i], NVG_PT_CORNER);
    }
...
void nvgStrokeEXWithPointList(NVGcontext* ctx, NVG_PointListElement* list, size_t count)
{
    NVGstate* state = nvg__getState(ctx);
    float scale = nvg__getAverageScale(state->xform);
    float strokeWidth = nvg__clampf(state->strokeWidth * scale, 0.0f, 200.0f);
...
//    }
    ...
    nvg__flattenPathsWithPointList(ctx, list, count);
