...
template <typename ARR, typename T> inline T* array_ptr_get(ARR* arr, usize idx)
{
    return &((*arr)[idx]);
}
...
struct alignas(16) Array_Slice {
    ...
    inline static Array_Slice_Raw to_raw(Array_Slice<T>* self)
    {
        return {.count = self->count, .data = reinterpret_cast<void*>(self->data), .type_size = sizeof(T), .end =
            (void*)(((uintptr)self->data) + (self->count * sizeof(T)))
        };
    }
    inline static Array_Slice_Raw to_raw(Array_Slice<T>* self, usize i, usize j_exclusive)
    {
#ifdef NO_ARRAY_BOUNDS_CHECK
        ASSERT_MSG(j_exclusive >= i, "error: arguments for Array_Slice to_raw() invalid i=[%llu], j_exclusive=[%llu]\n", i, j_exclusive);
#endif
        usize count = j_exclusive - i;
        return {.count = count, .data = reinterpret_cast<void*>( ((uintptr)self->data) + i * sizeof(T)), .type_size = sizeof(T), .end =
            reinterpret_cast<void*>(((uintptr)self->data) + (count * sizeof(T)))
        };
    }
    
    inline static Array_Slice<T> from_raw(Array_Slice_Raw* raw_slice)
    {
        Array_Slice<T> slice = {};
        slice.count = raw_slice->count;
        slice.data = reinterpret_cast<T*>(raw_slice->data);
        return slice;
    }
    
    inline static void to_buffer(Array_Slice<T>* self, T** buf, usize* count)
    {
        *count = self->count;
        *buf = self->data;
    }
    
    inline static Array_Slice<T> from_buffer(T* buf, usize count)
    {
        Array_Slice<T> slice = {};
        slice.count = count;
        slice.data = buf;
...
struct  alignas(16) Array {
    ...
    inline static Array_Slice<T> to_Array_Slice(Array<T, N>* self)
    {
        return Array_Slice<T>{self->count, self->data};
    }
    inline static Array_Slice<T> to_Array_Slice(Array<T, N>* self, usize i, usize j_exclusive)
    {
        return Array_Slice<T>{j_exclusive - i, self->data + i};
    }
    
    inline operator Array_Slice<T>(void)
    {
        return Array<T, N>::to_Array_Slice(this);
    }
...
TYPE_T_SIZE_N
void increment_count_checked(Array<T, N>* array, const usize count_increment);

TYPE_T_SIZE_N
void set_count(Array<T, N>* array, const usize count);
...
struct alignas(16) Dynamic_Array {
    ...
    inline static Array_Slice<T> to_Array_Slice(Dynamic_Array<T>* self)
    {
        return Array_Slice<T>{self->count, self->data};
    }
    inline static Array_Slice<T> to_Array_Slice(Dynamic_Array<T>* self, usize i, usize j_exclusive)
    {
        return Array_Slice<T>{j_exclusive - i, self->data + i};
    }
    ...
    inline operator Array_Slice<T>(void)
    {
        return Dynamic_Array<T>::to_Array_Slice(this);
    }
    ...
    constexpr const T& operator[](usize index, auto&&... args)
    {
#ifndef NO_ARRAY_BOUNDS_CHECK
        ASSERT_MSG(index >= 0 && index < this->count, "Index %llu is out-of-bounds ranges 0..<%llu", index, this->count);
#endif
        auto& ref = this->data[index];
        return ref.operator[](args...);
    }
    ...
    constexpr const T& operator[](usize index, auto&&... args) const
    {
#ifndef NO_ARRAY_BOUNDS_CHECK
        ASSERT_MSG(index >= 0 && index < this->count, "Index %llu is out-of-bounds ranges 0..<%llu", index, this->count);
#endif
    ...
    inline void increment_count(const usize count_increment)
    {
        increment_count(this, count_increment);
    }
...
TYPE_T
static inline void Array_init(Dynamic_Array<T>* array)
{
    init(array);
}
TYPE_T
static inline void Array_init(Dynamic_Array<T>* array, mem::Allocator& a)
{
    init(array, a);
}
...
//void init(Dynamic_Array<T>* array, usize count);
TYPE_T
static inline void Array_init(Dynamic_Array<T>* array, mem::Allocator& a, usize count)
{
    init(array, a, count);
}

TYPE_T
static inline void Array_init(Dynamic_Array<T>* array, mem::Allocator& a, usize count, usize capacity)
{
...
void increment_count_checked(Array<T, N>* array, const usize count_increment)
{
    ...
    array->count = (next_count <= count_increment) ? next_count : N;
}
...
Dynamic_Array<T> Dynamic_Array<T>::make_from_slice(Array_Slice<T>* array_slice)
{
    ...
    init_from_ptr(&array, array_slice->data, array_slice->count, array_slice->count);
