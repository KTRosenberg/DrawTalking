...
@implementation SD_Renderer_Metal_Backend {
    ...
    id<MTLFunction> _polygon_color_vertex_function;
    MTLRenderPipelineDescriptor* _p_color_descriptor;
    ...
    mtt::Dynamic_Array<usize> _render_pipelines_free_list;
    ...
    id<MTLFunction> _polygon_color_texture_vertex_function;
    MTLRenderPipelineDescriptor * _p_color_texture_descriptor;
...
//        }
        ...
        mtt::init(&_render_pipelines_free_list, _core_platform->core.allocator);
        ...
        _core_platform->renderer.depths_default = {nextafter(-1000.0f, -999.0f), nexttoward(1000.0f, 999.0f)};
...
- (void)_loadMetalWithView:(nonnull MTKView *)view;
{
    @autoreleasepool {
        ...
        _polygon_color_vertex_function = polygon_color_vertex_function;
        ...
        _p_color_descriptor = [polygon_color_PipelineStateDescriptor copy];
        ...
        
        {
            ...
            if (supports_metal_3) {
                ...
                
                {
                    ...
                    _polygon_color_texture_vertex_function = polygon_color_texture_vertex_function;
                    ...
                    _p_color_texture_descriptor = [polygon_color_PipelineStateDescriptor copy];
...
- (void)_updateUniforms {
    ...
    auto size = get_bounds(get_main_screen()).size;
    mat4 proj = m::ortho(0.0, size.width,
                           size.height, 0.0, -1000.0, 1000.0);
    //MTT_print("width %f, height %f\n", size.width, size.height);
...
- (void)update:(nonnull MTKView*)view {
    ...
    [command_buffer addCompletedHandler:^(id<MTLCommandBuffer> buffer) {
        ...
        core->time_milliseconds = current_time_ns / 1000000;
    ...
    @autoreleasepool {
        ...
        if (renderPassDescriptor != nil) MTT_LIKELY {
            ...
            self->active_viewport = _core_platform->core.viewport;
                ...
                for (usize cmd_idx = 0; cmd_idx < cmd_list_size; cmd_idx += 1)
                {
                    ...
                    case sd::COMMAND_TYPE_RENDER_PASS_BEGIN: {
                        if (cmd->render_pass.id != 0) {
                            active_viewport = cmd->render_pass.viewport;
                            ...
                            if (USE_MULTISAMPLING) {
                                if (Render_Target_is_msaa(rt)) {
                                    ...
                                    offscreen_renderpass_descriptor_msaa.colorAttachments[0].clearColor = MTLClearColorMake(cmd->render_pass.color_targets[0].clear_color[0],
                                                                                                                            cmd->render_pass.color_targets[0].clear_color[1],
                                                                                                                            cmd->render_pass.color_targets[0].clear_color[2],
                                                                                                                            cmd->render_pass.color_targets[0].clear_color[3]);
                                    ...
                                    offscreen_renderpass_descriptor.colorAttachments[0].clearColor = MTLClearColorMake(cmd->render_pass.color_targets[0].clear_color[0],
                                                                                                                            cmd->render_pass.color_targets[0].clear_color[1],
                                                                                                                            cmd->render_pass.color_targets[0].clear_color[2],
                                                                                                                            cmd->render_pass.color_targets[0].clear_color[3]);
                    ...
                    case sd::COMMAND_TYPE_VERTEX_BUFFER_SET: {
                        [renderEncoder setVertexBuffer: (__bridge id<MTLBuffer>)cmd->render_pass_buffer_set.buffer.buffer offset:cmd->render_pass_buffer_set.byte_offset atIndex:cmd->render_pass_buffer_set.slot];
                        continue;
                    }
                    case sd::COMMAND_TYPE_FRAGMENT_BUFFER_SET: {
                        [renderEncoder setFragmentBuffer: (__bridge id<MTLBuffer>)cmd->render_pass_buffer_set.buffer.buffer offset:cmd->render_pass_buffer_set.byte_offset atIndex:cmd->render_pass_buffer_set.slot];
                        continue;
                    }
                    ...
                    case sd::COMMAND_TYPE_COMPUTE_BUFFER_SET: {
                        [compute_pass.encoder setBuffer:
                         (__bridge id<MTLBuffer>)cmd->compute_pass.buffer_set.buffer.buffer
                                                offset:cmd->compute_pass.buffer_set.byte_offset atIndex: cmd->compute_pass.buffer_set.slot];
                        continue;
                    }
...
-(void)pause {
    [_view setPaused:YES];
    MTT_Core_pause(&self->_core_platform->core);
}
-(void)resume {
    self->prev_time_ns = mtt_time_nanoseconds();
    MTT_Core_resume(&self->_core_platform->core);
    [_view setPaused:NO];
}
...
+ (void)enableMTLFastMath:(MTLCompileOptions*)compile_opts {
...
- (sd::Shader_ID) rasterization_shader_make_with_builtin_vertex_function:(sd::Shader_ID) vertex_shader_function_id fragment_function_src:(NSString*_Nonnull)fragment_function_src fragment_function_name:(NSString*_Nonnull)fragment_function_name;
{
    @autoreleasepool {
        ...
        switch ((sd::PIPELINE_MODE)vertex_shader_function_id) {
            case PIPELINE_MODE_COLOR: {
                vertex_function = _polygon_color_vertex_function;
                desc = [_p_color_descriptor copy];
                break;
            }
            case PIPELINE_MODE_TEXTURE: {
                vertex_function = _polygon_color_texture_vertex_function;
                desc = [_p_color_texture_descriptor copy];
                break;
            }
            default: {
                return shader_id;
        ...
        [SD_Renderer_Metal_Backend enableMTLFastMath:compile_opts];
        compile_opts.preserveInvariance = YES;
        
        id<MTLLibrary> source_library = [_device newLibraryWithSource:fragment_function_src options:compile_opts error:&error];
        if (error) {
        ...
        id<MTLFunction> fragment_function = [source_library newFunctionWithName:fragment_function_name];
        ...
        uint64 next = 0;
        if (_render_pipelines_free_list.empty()) {
            next = _pipelines.size();
            mtt::append(&_pipelines, (id)pipeline);
        } else {
            next = _render_pipelines_free_list.back();
            mtt::pop(&_render_pipelines_free_list);
            _pipelines[next] = (id)pipeline;
        }
        
        return shader_id;
    }
}
- (sd::Shader_ID) rasterization_shader_make:(NSString*_Nonnull) vertex_src vertex_function_name:(NSString*_Nonnull)vertex_function_name  fragment_src:(NSString*_Nullable)fragment_src fragment_function_name:(NSString*_Nonnull)fragment_function_name
{
    @autoreleasepool {
        ASSERT_MSG(false, "not implemented");
        return sd::Shader_ID_INVALID;
    }
}
- (id<MTLRenderPipelineState>_Nullable) rasterization_shader_lookup_with_id:(sd::Shader_ID) shader_id
{
    if (shader_id >= _pipelines.size()) {
        return nil;
    }
    return _pipelines[shader_id];
}
- (void) rasterization_shader_destroy:(sd::Shader_ID)shader_id
{
    @autoreleasepool {
        _render_pipelines_free_list.push_back(shader_id);
        _pipelines[shader_id] = nil;
    }
}
- (sd::Shader_ID) rasterization_shader_replace_with_builtin_vertex_function:(sd::Shader_ID) vertex_shader_function_id fragment_function_src:(NSString*_Nonnull)fragment_function_src fragment_function_name:(NSString*_Nonnull)fragment_function_name with_shader_id:(sd::Shader_ID)shader_id;
{
    if ([self rasterization_shader_lookup_with_id:shader_id] == nil) {
        return [self rasterization_shader_make_with_builtin_vertex_function:vertex_shader_function_id fragment_function_src:fragment_function_src fragment_function_name:fragment_function_name];
    }
    ...
    @autoreleasepool {
        ...
        _pipelines[shader_id] = nil;
        _pipelines[shader_id] = (id)pipeline;
        
        return shader_id;
    }
}
- (sd::Shader_ID) rasterization_shader_replace:(NSString*_Nonnull) vertex_src vertex_function_name:(NSString*_Nonnull)vertex_function_name  fragment_src:(NSString*_Nullable)fragment_src fragment_function_name:(NSString*_Nonnull)fragment_function_name
{
...
- (ARSession*_Nonnull) ARSession_get {
    return self->_session;
}
- (void) ARSession_set:(ARSession*_Nonnull) session {
...
- (usize) max_texture_dimension_2 {
    // https://developer.apple.com/documentation/metal/mtldevice/3143473-supportsfamily
    return  ([_device supportsFamily:MTLGPUFamilyApple3]) ? 16384 : 8192;
}
