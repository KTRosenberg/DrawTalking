// Gotten from
// https://github.com/etaler/Etaler/blob/master/Etaler/Core/SmallVector.hpp,
// with some edits
...
private:
  ...
  ////////////typedef smallvec::AlignedCharArrayUnion<T> U;
  //typedef typename std::aligned_union<1, T>::type U;
  typedef Aligned_Bytes<T> U;
  U FirstEl;
...
void SmallVectorTemplateBase<T, isPodLike>::grow(size_t MinSize) {
  ...
  size_t NewCapacity = size_t(smallvec::detail::NextPowerOf2(CurCapacity + 2));
...
public:
  ...
  /*
    ...
    explicit SmallVector(const smallvec::iterator_range<RangeTy> &R)
        : SmallVectorImpl<T>(N) {
...
} // End smallvec namespace

namespace std {
/// Implement std::swap in terms of SmallVector swap.
template<typename T>
inline void swap(smallvec::SmallVectorImpl<T> &LHS, smallvec::SmallVectorImpl<T> &RHS) {
  LHS.swap(RHS);
}

/// Implement std::swap in terms of SmallVector swap.
template<typename T, unsigned N>
inline void swap(smallvec::SmallVector<T, N> &LHS, smallvec::SmallVector<T, N> &RHS) {
  LHS.swap(RHS);
}
}
