...
int il_push_back(IntList* il)
{
    ...
    if (new_pos > il->cap)
    {
        ...
        {
            il->data = (int*)malloc(new_cap * sizeof(*il->data));
            memcpy(il->data, il->fixed, sizeof(il->fixed));
        }
        else
        {
            // Otherwise reallocate the heap buffer to the new size.
            il->data = (int*)realloc(il->data, new_cap * sizeof(*il->data));
        }
...
enum
{
    ...
    // Stores the root_sxs of the node using a centered rectangle and half-size.
...
void qt_query(Quadtree* qt, IntList* out, float x1, float y1, float x2, float y2, int omit_element)
{
    ...
    if (qt->temp_size < elt_cap)
    {
        ...
        qt->temp = (char*)realloc(qt->temp, qt->temp_size * sizeof(*qt->temp));
