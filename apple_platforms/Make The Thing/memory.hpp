...
#ifndef NDEBUG
#define MEM_ASSERT_MSG(cond, msg, ...) do { \
if (!(cond)) { \
...
void Pool_Allocation_init(Pool_Allocation* pool_allocation, mem::Allocator backing, usize initial_count)
{
    static_assert(m::is_powerof2(ALIGNMENT), "alignment must be a power of 2");
...
template <class T> void deinit_deallocate(Allocator* a, T* mem)
{
    if (mem == nullptr) {
        return;
    }
