...
struct b2SizeMap
{
    constexpr b2SizeMap() : values()
    {
        ...
        {
            //MEM_ASSERT_MSG(j < BLOCK_SIZES_COUNT, "sizes mismatch\n");
            if (i <= BLOCK_SIZES[j])
            {
                values[i] = (uint8)j;
            }
...
void Arena_allocate_chunk_(Arena* arena, usize size)
{
    ...
    MEM_ASSERT_MSG(data != NULL, "memory arena allocation error (failed to allocate data) %s", __PRETTY_FUNCTION__);
...
{
    void* data = pool->backing_allocator.allocate(&pool->backing_allocator, pool->chunk_size);
    MEM_ASSERT_MSG(data != NULL, "memory pool fixed allocation error (failed to allocate data) %s", __PRETTY_FUNCTION__);
#ifndef NDEBUG
    const auto comp_first = (usize)data;
    const auto comp_second = (usize)align_up((usize)data, pool->block_align);
    MEM_ASSERT_MSG(comp_first == comp_second, "%llu mem does not match aligned %llu\n", comp_first, comp_second);
#endif
    ...
    MEM_ASSERT_MSG(((usize)curr) % pool->block_align == 0, "%s\n", "incorrect alignment");
    ...
    for (usize block_index = 0; block_index < pool->block_count - 1; block_index += 1) {
        ...
        MEM_ASSERT_MSG(((usize)next) % pool->block_align == 0, "%s\n", "incorrect alignment");
        MEM_ASSERT_MSG(((usize)next_val) % pool->block_align == 0, "%s\n", "incorrect alignment");
    ...
    *end = (uintptr)NULL;
    ...
    MEM_ASSERT_MSG(((usize)chunk) % pool->block_align == 0, "%s\n", "incorrect alignment");

    pool->chunk_list.next = chunk;
    MEM_ASSERT_MSG(((usize)chunk->next) % pool->block_align == 0, "%s\n", "incorrect alignment");
    
    // the free list should begin after the chunk header
    pool->free_list = allocation_begin;
    MEM_ASSERT_MSG(((usize)pool->free_list) % pool->block_align == 0, "%s\n", "incorrect alignment");
...
void* Memory_Pool_Fixed_init(Memory_Pool_Fixed* pool, Allocator backing, usize num_blocks, usize block_size, usize block_align)
{
    ...
    pool->chunk_count = 0;
    pool->chunk_size = align_up(sizeof(Memory_Chunk), block_align) + align_up(num_blocks * actual_block_size, block_align);
    
#ifndef NDEBUG
    pool->mem_debug = new mtt::Set_Stable<void*>();
#endif
...
void* Memory_Pool_Fixed_allocate_proc(void* state, usize size)
{
    ...
    if (size > pool->block_size) {
        return pool->backing_allocator.allocate(&pool->backing_allocator, align_up(size, pool->block_align));
    }
    void* ptr = NULL;
//#ifndef NDEBUG
...
//#endif
    ...
    MEM_ASSERT_MSG((next_free) % pool->block_align == 0, "%s\n", "incorrect alignment");
    // get next free block
    ptr = pool->free_list;
    MEM_ASSERT_MSG(((usize)ptr) % pool->block_align == 0, "%s\n", "incorrect alignment");
    // set free list to refer to the next free block
    pool->free_list = (void*)next_free;
    MEM_ASSERT_MSG(((usize)pool->free_list) % pool->block_align == 0, "%s\n", "incorrect alignment");
//    if ((((usize)pool->free_list) % pool->block_align) != 0) {
//        printf("%s, %llu %llu\n", "alignment wrong", (usize)pool->free_list, pool->block_align);
//        abort();
//    }
...
#ifndef NDEBUG
    ((mtt::Set_Stable<void*>*)pool->mem_debug)->insert(ptr);
#endif
...
#ifndef NDEBUG
    auto* mem_debug = (mtt::Set_Stable<void*>*)(pool->mem_debug);
    MEM_ASSERT_MSG(mem_debug->contains(memory), "%s\n", "should exist");
    mem_debug->erase(memory);
#endif
    
    if (count > pool->block_size) {
        return pool->backing_allocator.deallocate(&pool->backing_allocator, memory, count);
    }
    
    uintptr* next = (uintptr*)memory;
    MEM_ASSERT_MSG(((usize)memory) % pool->block_align == 0, "%s\n", "incorrect alignment");
    MEM_ASSERT_MSG(((usize)next) % pool->block_align == 0, "%s\n", "incorrect alignment");
    MEM_ASSERT_MSG(((usize)pool->free_list) % pool->block_align == 0, "%s\n", "incorrect alignment");
...
{
    Allocator* a = &pool->backing_allocator;
    ...
    while (curr != NULL) {
        ...
        curr = curr->next;
#ifndef NDEBUG
        MEM_ASSERT_MSG(mem_debug->contains(curr), "%s\n", "should exist");
#endif
...
{
    MEM_ASSERT_MSG(m::is_powerof2(alignment), "%s\n", "invalid alignment");
...
void* Fixed_Buckets_Allocator_allocate_proc(void* state, usize size)
{
    ...
    MEM_ASSERT_MSG(0 <= index && index < BLOCK_SIZES_COUNT, "block size mismatches!\n");
    //MTT_print("    index %llu\n", index);
    ...
    if (!buckets->is_init[index]) {
        ...
        Pool_Allocation_init(bucket, buckets->backing, buckets->initial_counts, size_adjusted, 256);
    }
    //MTT_print("}\n");
...
void Fixed_Buckets_Allocator_deallocate_proc(void* state, void* memory, usize size)
{
    ...
    MEM_ASSERT_MSG(0 <= index && index < BLOCK_SIZES_COUNT, "block size mismatches!\n");
    //MTT_print("    index %llu\n", index);
    MEM_ASSERT_MSG(buckets->is_init[index], "Should be initialized!\n");
